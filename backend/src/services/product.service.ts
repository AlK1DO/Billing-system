...
interface ProductQuery { search?: string; categoryId?: number; status?: 'active' | 'inactive'; page: number; limit: number; }
interface CreateProductData { name: string; sku: string; description?: string; categoryId?: number | null; price: number; cost?: number; stock: number; minStock?: number; imageUrl?: string; status?: 'active' | 'inactive'; companyId: number; }
...
export async function getProducts(companyId: number, query: ProductQuery) {
  const where: Prisma.ProductWhereInput = { companyId, ...(query.status && { status: query.status }), ...(query.categoryId && { categoryId: query.categoryId }), ...(query.search && { OR: [{ name: { contains: query.search } }, { sku: { contains: query.search } }, { description: { contains: query.search } }] }) };
  const [total, products] = await prisma.$transaction([
    prisma.product.count({ where }),
    prisma.product.findMany({ where, include: { category: { select: { id: true, name: true } } }, orderBy: { name: 'asc' }, skip: (query.page - 1) * query.limit, take: query.limit }),
  ]);
...
export async function updateProduct(id: number, companyId: number, data: UpdateProductData) {
  ...
  if (data.categoryId != null) await ensureCategoryBelongsToCompany(data.categoryId, companyId);
  return prisma.product.update({ where: { id }, data, include: { category: { select: { id: true, name: true } } } });
}
